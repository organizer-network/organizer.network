		...
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				if (context.thread) {
					...
					let placeholders = [];
					for (let i = 0; i < context.ids.length; i++) {
					...
					let before_clause = '';
					let values = context.ids.slice();
					if (before_id) {
						before_clause = 'AND message.id < $' + (context.ids.length + 1);
					...
					query = await self.query(`
						...
						FROM message, person
						WHERE message.context_id IN (${placeholders})
						  ${before_clause}
						  AND message.person_id = person.id
	...
	get_latest_messages: (contexts) => {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				let query = await self.query(`
					SELECT message.id, message.context_id, message.in_reply_to,
					       message.content, message.created, message.updated,
					       person.name AS person_name, person.slug AS person_slug
					FROM message, person
					WHERE message.person_id = person.id
					  AND message.context_id IN (${placeholders})
					  AND message.in_reply_to IS NULL
					ORDER BY message.created DESC
					LIMIT 20
				`, ids);

				let messages = await self.add_message_details(query.rows);
				let groups = {};

				for (let group of contexts) {
				...
				let total_messages = query.rows[0].total_messages;
	...
	get_context_latest_message: function(context_id) {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				let latest = query.rows[0];

				while (latest && latest.in_reply_to) {
